...
## Role and scope
- Owner outcome: disponer de Nano Monitor 1.4.0-r14 operativo en OpenWrt 24.10.5 con cuota diaria individual, límites por equipo, recuperación después de reinicios y una vista LuCI realmente rápida.
...
- `luci-app-nano-monitor_1.4.0-r14` fue compilado, instalado y verificado; SHA-256 del IPK: `434bd67ff49bcbe928adad1928234ead83dfb06d6d2c449f223895cefa1e1c69`.
...
- Causa raíz del rendimiento hallada: el tráfico originado por el router hacia `br-lan` no pasaba por el hook `forward` y caía en la clase default de 64 Kbit/s. r14 añade y verifica un hook `output` limitado a `br-lan` que lo envía a la clase local/prioritaria sin alterar el tráfico WAN reenviado.
- Medición durable tras instalar r14 y reiniciar shaper: recurso LuCI de 14,535 B bajó de 1.956 s a 0.023 s; raíz 0.008 s y login no autenticado 0.576 s. Accounting sigue `active/verified`; shaper `active/applied` 768/128 y radio `00`/167/HT20 preservada.
...
- Commits r13 `e888b7f` y `3be65d6` publicados en `fork/nano-m5-upper-5ghz`.
- Fix r14 en `5300f89`, revisado independientemente con resultado `PASS`.

## Open decision / acceptance boundary
- Resultado técnico r14 validado; rendimiento real `pending_owner_acceptance`.

## Next safe action
- Pedir al owner una recarga `Ctrl+F5` para medir la mejora real desde su navegador.
